...
Page copy is **inlined directly in each page** - there are no `messages/*.json` files and no translation layer. The home page's list content lives in typed local arrays at the top of [src/app/page.tsx](src/app/page.tsx) (`work`, `projects`, `education`, `social`); editing those rows means editing the arrays (or adding a new `Row`). The **intro prose is not in any array** - the `<h1>`, the `AI Engineer · Software Builder · Founder` tagline, the intro paragraph, and the "Work with me" body paragraph are literal JSX in the `Home` component's markup, so edit them in place (grepping the arrays for that text finds nothing). The `metaDescription` string (feeding both the page metadata and the JSON-LD `description`) is a separate `const` near the top.
